...
import type { PoolStateStore } from '../../shared/pool-state-store.ts';
import type { WorkerManager } from '../../orchestrator/worker-manager.ts';
...
export function createAdminApp(input: ApiServerInput): Hono {
  const { store, workerManager, crossChainDetector } = input;
  const app = new Hono();
  ...
  app.get('/pools', (c) => {
    ...
    if (symbol) {
      ...
      pools = pools.filter((p) => p.tokenPair.token0.symbol === sym || p.tokenPair.token1.symbol === sym);
    }
    ...
    const summaries = pools.map((p) => ({
      ...
      ...('reserve0' in p ? { reserve0: p.reserve0.toString(), reserve1: p.reserve1.toString() } : {}),
      ...('sqrtPriceX96' in p ? { sqrtPriceX96: p.sqrtPriceX96.toString(), tick: p.tick, liquidity: p.liquidity.toString() } : {}),
      ...('spotPrice0to1' in p ? { spotPrice0to1: p.spotPrice0to1, spotPrice1to0: p.spotPrice1to0 } : {}),
    }));
...
// ════════════════════════════════════════════════════════════════
// START ADMIN SERVER (Bun native HTTP + WebSocket)
// ════════════════════════════════════════════════════════════════

export function startAdminServer(port: number, deps: AdminDeps): { server: ReturnType<typeof Bun.serve>; app: Hono } {
